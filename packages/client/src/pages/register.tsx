...
export default function Register() {
    ...
    const onSubmit = async (values: typeof defaultValues) => {
        ...
        try {
            const res = await registerUser({ ...data, tier: parseInt(data.tier) }).unwrap()
            ...
            const error = (err as FetchBaseQueryError).data as ExcludeSuccess<RegisterUserResponse>
    ...
    return (
        <>
            <div className={`mb-[150px] bg-[url("/register-page-background.png")] h-[1028px]`}>
                <div className={"pt-[120px] max-w-fit mx-auto"}>
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="max-w-[860px] mx-auto mt-[100px]">
                        <div className={`flex overflow-hidden [&>*]:min-w-full [&>*]:transition-all`}>
                            <div style={{ transform: `translateX(-${(currentStep - 1) * 100}%)` }}> {/* First Step */}
                                <div className={"max-w-[600px] mx-auto"}>
                                    <div className={"flex justify-between"}>
                                        <Input autoComplete="off" classes={{ container: 'max-w-[255px] w-full', input: 'w-full' }} errors={errors} register={register('username', { required: "Field is reqired" })} placeholder="Username" />
                                        <Input autoComplete="off" classes={{ container: 'max-w-[255px] w-full', input: 'w-full' }} errors={errors} register={register('email', { required: "Field is required" })} placeholder="Email" />
                                    </div>
                                    <div className={"flex justify-between mt-[60px]"}>
                                        <Input autoComplete="off" classes={{ container: 'max-w-[255px] w-full', input: 'w-full' }} register={register('password', { required: "Field is required" })} errors={errors} placeholder="Password" />
                                        <Input autoComplete="off" classes={{ container: 'max-w-[255px] w-full', input: 'w-full' }} register={register('confirmPassword', { required: "Field is required" })} errors={errors} placeholder="Confirm password" />
                                    </div>
                            ...
                            <div style={{ transform: `translateX(-${(currentStep - 1) * 100}%)` }}>
                                <div className="text-high-emphasis p-10 bg-[#24201A] rounded-[20px] mt-[200px]">
                                    ...
                                    <Button styleType={"filled"} onClick={redirectToLoginPage}>Go to login page</Button>
                                </div>
                            </div>
                        </div>
                    </form>
